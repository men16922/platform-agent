...
- `make check` (pytest) → **795 passed, 1 skipped** (2026-07-17, 225.47s) — **⑧ 안전 서브셋: A2A 위임 아웃바운드 sanitize+cap**: `supervisor.sanitize_instruction`(control-char strip[tab/newline 유지]·4000자 cap·적용 transform trace) + `handle` 아웃바운드 배선(분류는 원문 유지)·`trace` 타입주석 정정. 위임 계약/동작 변경 4건(구조화 페이로드·저-confidence 게이트·최소권한 힌트·경계 smell-test)은 `docs/plans/a2a-delegation-hardening.md`로 분리=**승인 대기**. 비파괴 +5 test(790→795).
