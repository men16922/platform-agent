...
- (이전 이력: gate **1496** 이하 · 2026-07-10~29 → `docs/archive/status-baseline-2026-07.md`
...
- **차단 없는 잔여 = 소진(2026-07-28~29) → M13, 10건** — 전부 같은 결함 축이었고 **열 번 다
  테스트는 초록, 라이브만** 드러냈다(상세 → `COMPLETED_SUMMARY` M13). 아홉은 "선언되고 저장되고
  **아무도 읽지 않는다**", 열 번째는 **반대 방향**("읽는데 아무도 안 씀") — 이쪽이 더 안 보인다:
  생산자가 여럿이면 **하나만 침묵해도** 나머지가 정상을 계속 증명해주고, 기존 스윕은 이 방향을
  구조적으로 못 본다(선언·채워짐·읽힘이 전부 성립) → `scripts/find_unwritten_keys.py`.
  **현행 교훈 3종**: 부재보다 **그럴듯한 기본값**이 오래 산다(MTTR 0.0) · **투영/스키마 계층도
  소비자**다 · 가드는 **파생**시켜라(열거하면 다음 필드에 실패). 남은 잔여는 전부 **결정 대기**
  이지 작업 대기가 아니다(아래 3건).
