...
- `make check` → **1825 passed, 2 skipped** (2026-08-12, +36) — 게이트 줄의 **`1 skipped`를
  이름 불렀다**. skip 자체는 정당했지만(지우면 red), 그걸 읽다 `test_walk_all_steps`가
  **선언된 16스텝 중 10개(62%)**만 걷는 걸 찾았다 — 안 걷는 6개는 **에스컬레이션 분기 전부**.
  구현은 멀쩡(24/24 resolve), **테스트가 안 걸었을 뿐**. 변이 4건 red.
  증거 `runbook-walk-skipped-the-escalation-branch.log`.
- `make check` → **1789 passed, 1 skipped** (2026-08-11, 1737→**+52**, **로컬 ↔ CI 일치**;
  #24~#28 **다섯 지점 전부** 숫자가 같다) — 리포트 스트림 전수 훑기(문 넷). 경위·산출·남긴
  경계는 **M17이 권위**. 변이 45건 red, 생존 0.
- `make check` → **1737 passed, 1 skipped** (2026-08-09, +18, **로컬 ↔ CI 일치**) — 프로브를
  **정기 실행**하려다 **진짜 구속 조건**을 만났다: LaunchAgent는 `~/Desktop` 아래 레포를
  **읽지 못한다**(macOS TCC). 뚫으려면 `/bin/zsh`에 Full Disk Access = 모든 zsh 스크립트에
  전체 디스크 → **안 했다**. 대신 **이미 허가된** 대화형 셸에 태웠다(터미널당 하루 한 번) —
  즉 **터미널을 안 열면 검사도 없다**. 증거 `spend-watch-launchd-blocked-by-tcc.log`.
...
12. **게이트의 초록에는 조건이 붙는다 — 넷이 같은 계열(2026-08-08~11)** —
   ...
   **달력이 움직이기 전까지만** 참이다(07-29 픽스처 + 7일 창 → 08-05 만료). **게이트 숫자는
   ...
   → **새 가드는 지워 보고 red를 확인할 것.** ⚠️08-12에 **둘째 얼굴**이 나왔다 — 분기를
   **깨져야만 열리는 플래그**로 걸면 그 분기는 *원리상 도달 불가*다(`previous_step_failed`가
   `except` 안에서만 True → 에스컬레이션 6스텝을 아무도 안 걸었다). **조건 축은 값을 넣어
   명시적으로 걸 것.** ④**관측 지점 — 08-11 전수 훑기(상세는 M17)**:
