...
6. **공급망 보증은 0이다 — "CI/사람용 게이트까지"가 틀렸다(2026-08-08 측정)** — 있는 건 **검증
   도구 하나**(`scripts/verify_image_signature.py`)뿐이고 **게이트는 없다**: ①`cosign sign`이
   레포에 **0건** — 서명을 만든 적이 없다 ②`.github/workflows/`가 **없다** — "CI 게이트"는 돌 수
   없는 단계였다 ③검증기의 유일한 호출자는 **자기 테스트**다(D39와 같은 모양) ④차트가
   `platform-agent:0.1.0`을 **레지스트리 호스트 없는 맨 태그**로 배포하고 `digest: ""`다 —
   서명은 레지스트리의 **다이제스트 옆**에 사는 아티팩트라 **놓일 주소조차 없다**.
   **어드미션 승인의 전제가 뒤집힌다**: policy controller를 지금 넣으면 서명을 강제하는 게
   아니라 **모든 워크로드를 거부**한다(찾을 서명이 없으므로). 선행은 **서명 경로**다.
