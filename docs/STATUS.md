...
- `make check` → **1779 passed, 1 skipped** (2026-08-11, 1737→**+42**, **로컬 macOS·py3.13
  ↔ CI 1779 일치**; 세 지점 전부 숫자 일치, PR #22·#24·#25·#26) — **리포트가 독자에게 갈린 채 도착하고
...
12. **게이트의 초록에는 조건이 붙는다 — 넷이 같은 계열(2026-08-08~11)** —
   ...
   **달력이 움직이기 전까지만** 참이다(07-29 픽스처 + 7일 창 → 08-05 만료). **게이트 숫자는
   ...
   → **새 가드는 지워 보고 red를 확인할 것.** ④**관측 지점 — 08-11 전수 훑기로 소진**:
   **가드는 독자가 읽는 그 물건에 대고 물을 것**(`capsys`가 스트림을 갈라 주므로 `.err`에
   묻는 테스트는 독자의 사본이 갈라진 걸 원리상 못 본다). 훑기의 결론 셋 —
   ⓐ**결함을 그 그림자로 세지 말 것**: 잘못 물은 가드는 3건인데 깨진 프로그램은 8건이고
   그중 둘은 **테스트에 이름조차 없었다**(→ 대상은 가드 목록이 아니라 프로그램).
   ⓑ**"stderr 금지"가 아니다**: 독자가 파서면 의무가 거꾸로 선다 → `scripts/*.py` 22개를
