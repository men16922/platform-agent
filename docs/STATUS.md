...
- **M13이 반대 방향으로도 열렸다(2026-07-29, `db41874`)** — 앞의 아홉은 "선언됐는데 아무도
  안 읽음"이었고 이번은 **"읽는데 아무도 안 씀"**이다. 부재는 이쪽이 더 안 보인다: 생산자가
  여럿일 때 **하나만 침묵하면** 나머지가 정상 동작을 계속 증명해준다. 기존 스윕은 이 방향을
  구조적으로 못 본다(필드는 선언·채워짐·읽힘 전부 성립) → `scripts/find_unwritten_keys.py`.
  **새 교훈**: 절반씩은 각각 방어 가능한데 **겹칠 때만** 터지는 결함이 있다 — writer 누락은
  과소보고일 뿐이고 reader의 `{...latest}`도 합리적인데, 둘이 만나면 **삭제**가 된다.
