...
- `make check` (pytest) → **669 passed, 1 skipped** (2026-07-14) — **Agent Runtime 호스팅 어댑터 3종**(신규 `adapters/runtime/`: AWS AgentCore(boto3)·GCP Agent Engine(vertexai)·Azure Foundry(azure-ai-projects), plan-first/approved-gated·읽기전용 preflight·teardown 승인 강제, +20 test). **AWS·GCP 실 배포 라이브 E2E 완결(2/3)**: 어댑터 create→READY/DEPLOYED→invoke/query(실 Claude/Gemini 응답)→teardown, 즉시 삭제(각 <$0.50). Azure는 Foundry 프로젝트 생성(과금) 선행이라 대기. 패키징: `infra/agentcore/`(arm64 컨테이너)·`infra/agentengine/`(custom-template).
