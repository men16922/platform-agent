...
> **무과금으로 이어서 할 것**: `NEXT_PLAN`의 capability 스캔 잔여 **ⓐ·ⓒ**(정책 판단 필요 —
> **ⓑ는 08-15에 닫혔다, 틀린 기록이었다**) · "런북 walk가 남긴 ②"(`severity` 축) ·
> `slack_live_approval`(**Slack 데모 선행**).
>
...
> ⚠️ **"안 봤다"를 시험하면 결함이 아니라 범위가 나올 때도 있다**(08-13에 두 번). ⚠️**형제 집합은 세는 순간 전부 셀 것** — 08-13에 세 번(**세 번째는 그걸 찾으려고 만든 스윕 안에서**), **08-15에 네 번째**(두 파일에 같은 필터를 넣었는데 **한쪽만 import가 되돌아가** 죽어 있었다). ⚠️**"선언됐는데 안 읽힌다"는 자동으로 결함이 아니다** — 기준은 **읽는 쪽의 provider 간 비대칭**이다(`provider` 필드를 읽기 시작하면 GCP/Azure는 전부 폴백으로 떨어진다).
> 직전 세션 → **M19**(`resource_types`가 선언만 되고 안 읽혔다 — PR #33·#34, +32) · **M18**(형제 집합, D47) · **M17**(리포트 스트림) · **M16**(비용 관측, D44~D46).
>
...
- **검증:** `make check` → **1894 passed, 2 skipped** (**2026-08-15, 로컬 macOS·py3.13 · CI도 1894 — 숫자 일치**; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M19**에 있다 — 여기에 다시 적지 말 것.
