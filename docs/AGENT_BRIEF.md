...
> ▶ NEXT SESSION: **Phase 2 잔여** — faked managed 디스크립터로 `applicable=false` 검증 · DR 재구축 라이브 확인. 그 다음은 Phase 3(인가 강화). 설계 `docs/plans/2026-07-21-multi-tenant-env-addons.md`(v5). **주의**: 새 애드온을 테넌트 네임스페이스에 넣을 땐 PSS restricted 통과를 `kubectl apply --dry-run=server`로 먼저 확인할 것 — 차트 기본값은 거부되는데 **Argo는 Synced로 보인다**. **직전 완료**: values seam+PSS/PVC 수정(`01d3c6d`) · 삭제 cascade(`e1ea15f`) · capability scope 축(`bb7a819`) · ⑥ NetworkPolicy 활성화(`3dbc572`) · push 2축 수집기+대시보드 스위처(`b2b52fc`) — gate **1281**(미푸시 10커밋). kind에 acme-dev(4ns)·globex-dev(1ns) + NetworkPolicy 5개. MLX는 내려둠.
>
