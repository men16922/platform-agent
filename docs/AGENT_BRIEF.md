...
> ▶ NEXT SESSION: **차단 없는 잔여는 소진됐다(2026-07-28). 남은 잔여는 전부 결정 대기다.** ①**"배포는 어느 테넌트 소유인가"가 두 항목을 동시에 막고 있다** — deployments/activities 파티션은 물론 **모델 호출 rate limit도** 여기 묶여 있다(로컬 모델 호출자는 `local_deployer`/`strands_deployer` 둘뿐, 둘 다 배포 경로. 배포 요청엔 테넌트가 없고 `setup_tenancy`는 **모델이 부르는 도구** — 테넌트가 추론의 **입력이 아니라 출력**이라 신원 전파 자체가 성립 안 함. 발명 금지) · ②**무스코프 MCP 읽기가 여전히 ambient**(검증된 익명 kagent 왕복을 살리려는 의도적 예외 — 닫으려면 kagent 경로에 스코프 선행) · ③**Capsule `limitRanges` 이관 경로 결정**(`GlobalTenantResource`=클러스터 스코프라 **D30 위반**, `TenantResource`=SA+RBAC 새 권한 표면). 그 다음은 **Phase 4**(managed, billable) 또는 **Phase 5**(레지스트리 쓰기 → D32 재검토 조건). **주의**: 카드/스키마/문서에 **집행하지 않는 것을 광고하지 말 것** · 가드를 쓰면 **호출부에서 반증할 것**(이번에 새 테스트 20건이 전부 통과하는데 호출부만 되돌아간 상태였다 — 라이브만 잡았다) · 푸시/네트워크 신규 필드는 항상 optional+폴백 · 대시보드는 `dashboard/AGENTS.md`대로 **Next 문서 먼저** · 자격증명이 테넌트-바운드인 것은 **온프렘뿐**(D31) · 파티션된 읽기 경로는 **둘뿐**(플릿·인시던트). **직전 완료**: 잔여 4건(grant 레지스트리 대조 · 선택 불가 런북 4개+티어 셰도잉 · Capsule metadata 이관 · executor span) — gate **1454**. **새로 보인 것**: 선택 가능해진 런북 4개가 **온프렘 알람엔 안 걸린다**(디텍터가 alertname을 버리고 `metric_name=availability`로 정규화 — 매칭 설계 결정).
>
