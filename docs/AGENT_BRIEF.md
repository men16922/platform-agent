...
> ▶ NEXT SESSION: **첫 행동 = `spend-check`의 `MEASURABLE` 분기가 금액을 대신 묻게 하기**(`NEXT_PLAN`의 그 항목 — 설정문서 §4가 *"데이터가 생기면 그때"*로 미뤄 둔 것이고 **09-02에 그 조건이 충족됐다**, M49). ⚠️**`cost`만 더하면 총사용액이지 청구액이 아니다** · ⚠️**`credits`를 펼쳐 더해도 "≈₩0"은 요율이 아니다** — 인쇄하는 답에 **크레딧 종류**가 같이 나와야 읽는 사람이 안 속는다 · ⚠️**BQ 질의는 스캔 과금**이다(프로브가 돌 때마다 무는 것이 CE를 청구서의 84%로 만든 방식이다). ⛔**GCP `numRows` 판정은 09-02에 끝났다 — 다시 열지 말 것**: **730행**·프로브 `MEASURABLE`, 총사용액 **₩67.87** → 청구 **≈₩0**인데 **상쇄의 99.4%가 `FreeTrialUpgrade` `PROMOTION` 크레딧**이다 ⇒ ⛔*"GCP는 어차피 공짜"*로 요약 금지(절벽은 **크레딧 잔액**이고 **소진 시점은 내보내기가 말해 주지 않는다**). ⚠️**런레이트는 아직 못 잰다**(창 **3일**·마지막 날 미완 · 총액의 **81%가 하루치 4행** · **90%가 다른 프로젝트 것**, platform-agent 몫 **₩6.55**) ⇒ **며칠 뒤 재조회가 무료인 다음 측정**. ⛔**09-01의 세 결함도 닫혔다 — 다시 열지 말 것**(M46·M47, archive 상세가 권위). 그다음 = **Phase 4 4b**(billable, **별 승인**) · **Phase 5 attach UI**(여는 조건 = **파이썬 플래너를 어디서 돌릴지**). ⚠️**셋 다 '쟀다'지만 답의 모양이 다르다 — 하나로 요약하지 말 것**(AWS는 **재측정 자체가 청구서** · Azure는 **간격을 지키면** · GCP는 **크레딧이 만든 ₩0**): 전문은 **`STATUS` Risk 4가 권위**다.
...
> 직전 세션 → **09-02**: `[auto]` 넷(M47 Azure 간격 · M48 인쇄 경로 · 문서 셋 압축) + **GCP 판정**(M49). ⛔**상세를 여기 다시 적지 말 것** — `PROGRESS_LOG` 상단이 권위다. ⚠️**09-01(일곱 묶음, M40~M46)에서 배운 것 둘**(**`| head -N`이 찾던 파일을 잘랐다** · **`AccessDenied`는 부재의 증거가 아니다**)의 전문은 `docs/archive/brief-detail-2026-09.md`.
>
