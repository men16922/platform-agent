...
> ▶ NEXT SESSION: **Phase 3(인가 강화)** 착수 — 설계 `docs/plans/2026-07-21-multi-tenant-env-addons.md`(v5). **Phase 2는 완결**(M11, gate 1290): tenancy+Capsule · ⑥ 데이터플레인 격리 · push 2축 수집기+대시보드 스위처 · capability scope 축 · 삭제 cascade · values seam · managed 경로 · DR 재구축까지 라이브 확인. **주의**: 새 애드온을 테넌트 네임스페이스에 넣을 땐 PSS restricted 통과를 `kubectl apply --dry-run=server`로 먼저 확인할 것(차트 기본값은 거부되는데 **Argo는 Synced로 보인다**). gate **1290**(미푸시 12커밋). kind에 acme-dev(4ns)·globex-dev(1ns) 모두 adopted and bounded + NetworkPolicy 5개. MLX는 내려둠.
>
