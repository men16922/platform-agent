...
> ▶ NEXT SESSION: **첫 행동 = `onprem` extra의 `mlx-lm`**(`NEXT_PLAN` 08-30 — 승인 게이트 없는 유일한 코드 항목이고 실측이 있다: **아무도 그 extra로 안 깔고** `src/`는 **임포트조차 안 한다** · `gate.yml`의 근거 *"linux resolve 불가"*는 **지금 거짓**). 그다음 = **정적검사를 게이트에 넣을지**(lint 20건·mypy 253건이 각각 선행 — **레포의 결정**). ⛔**4a는 접혔다(2026-09-01, 사용자 결정 — M40)**: 워크스페이스·IAM 사용자·키 **셋 다 삭제**, 8리전 스윕 0. ⚠️**$0.00이 접은 이유가 아니다** — 대가는 **장기 키**였고 D50이 미리 그렇게 적었다. ⚠️**D48은 안 죽었다** — 그래서 `test_amp_cost_handles`를 **지우지 않고** 계약을 함수로 빼 **합성 표**에 물렸다(+7). ⚠️**첫 판은 규칙이 다른 규칙의 그림자로 세어져 있었다**(와일드카드 검사를 지워도 초록) — **결함을 그 그림자로 세지 말 것**의 재발. ⚠️**`AccessDenied`를 부재의 증거로 읽지 말 것** — 증거는 **소유 사용자의 부재**다. ⛔**Azure executor는 배선됐다(08-30, 승인 후 — M39)**: 게이트가 **2337→2332로 내려갔고 그게 옳다**. ⚠️**"이제 Azure는 안전"으로 요약 금지** — 열린 건 **경로**고 blast radius 0은 **오늘의 사실**이다. ⛔**4a의 결론은 그대로다** — `docs/plans/2026-08-15-4a-remote-write-allowlist.md` **§10·§11이 권위**:
...
> 직전 세션 → **09-01**: **4a를 접었다**(셋 다 삭제) + 08-30 묶음 랜딩(PR #57 = M39 배선 + 비용 실측). ⛔**상세를 여기 다시 적지 말 것** — `PROGRESS_LOG` 상단과 `docs/archive/progress-2026-08.md`가 권위다. ⚠️**배운 것 둘**: **`| head -N`이 찾던 파일을 잘랐다**(docs/가 infra/보다 먼저 정렬돼 "레포엔 없다"고 쓸 뻔 — 세는 함정은 도구만이 아니다) · **`AccessDenied`는 부재의 증거가 아니다**(권한이 없어 나온 답이라 대상이 살아 있어도 같다 — 429가 *"'0'이 아니다"*였던 것과 같은 모양).
>
...
- **검증:** `make check` → **2339 passed, 2 skipped** (**2026-09-01 로컬 macOS·py3.13** · CI도 **같은 게이트**, ubuntu·py3.13 — **CI 숫자는 PR 체크가 권위**, 여기 복제 금지; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). ⚠️**08-30엔 2337→2332로 내려갔다**(M39, 정당화 7건 −14 / 새 가드 +9); **09-01의 +7은 4a를 접으며 가드를 지우지 않고 합성 표에 물린 것**(M40). **결함이 닫히면 그 결함을 설명하던 줄도 사라진다 ⇒ 단조증가 지표가 아니다.** 내역은 `STATUS` Baseline. CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M40**에 있다 — 여기에 다시 적지 말 것.
