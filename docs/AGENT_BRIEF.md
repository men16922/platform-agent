...
> ▶ NEXT SESSION: **결정 5는 닫혔다(2026-07-31, D38) — A·B 둘 다 실행.** 남은 것은 **결정 3건(2·3·4) + 승인 3건**. ⓪**가장 값싼 다음 수는 결정 2**(무스코프 MCP 읽기) — **A가 절반을 풀었다**: 막던 건 "`MCPServer(scope=...)`를 구성할 스코프가 프로덕션에 없다"였고 이제 `attest_decision`이 있다. 남은 건 kagent 경로가 **어떤 인가로** 스코프를 받느냐. 읽기가 무해해서 열어둔 게 아니다(남의 로그 = 유출). 그다음 **결정 4**(k3s: 집행은 라이브 증명, 시맨틱은 피어 테넌트 부재로 미증명 — globex/prod 생성 = 인프라 결정) · **결정 3**(Capsule `limitRanges`: 클러스터 스코프=D30 위반 vs SA+RBAC 새 표면). **승인 3건**: 실 DynamoDB 왕복 · GCP/Azure 보관 켜기(실 데이터 삭제) · Cosign 어드미션. ⚠️**핵심 주의**: 스코프도 배포 신원도 **옵트인**이다 — `make scope-credentials`·`make deploy-identity`를 안 하면 인시던트는 전부 거부되고 배포는 ambient(cluster-admin)로 돈다. 어느 상태인지 묻는 법: `python scripts/probe_scope_reachability.py` · `make deploy-identity-check`. **"설정하면 집행되고, 안 하면 조용히 안 된다"를 "집행된다"로 줄여 쓰지 말 것.** 그 밖의 주의: 카드/스키마/문서에 **집행하지 않는 것을 광고하지 말 것**(배포 신원은 **무엇을** 할 수 있는지만 좁힌다, **누구의** 네임스페이스인지가 아니다 — 테넌트 격리라 부르면 안 된다) · 가드는 **파생시켜라, 열거하지 말고** · **읽는 것과 돌리는 것은 다르다**(AST가 호출부를 봐도 이름이 resolve되는지는 모른다) · 푸시/네트워크 신규 필드는 항상 optional+폴백 · 대시보드는 `dashboard/AGENTS.md`대로 **Next 문서 먼저**. **직전 완료**: **결정 5 A·B**(`b92b54e`+`2a21b86`, gate **1596**) — **두 경로가 반대 방향으로 고장 나 있었다**. 인시던트 경로는 게이트가 옳은데 **생산자가 테스트뿐이라 열 수 없었고**(라이브 시 전부 거부), 배포 경로는 **가드 없이 cluster-admin**이었다(`get secrets -A`=yes). B: seam 하나로 고정 + RBAC(**허용은 어댑터가 부르는 kubectl에서 파생**, **금지는 명시**) — 라이브에서 축소 신원으로 실 배포·롤백이 되면서 위험 동사는 전부 API 서버가 Forbidden. A: `attest_decision()`을 **인가가 성립하는 두 지점**에서 호출(파킹 시점 서명은 **아무도 승인하지 않은 행동을 attest**한다) + 테넌트 자격증명 민팅(Phase 1a 증거의 그것은 **손으로 만들어 커밋된 적이 없었다** = 시연은 되고 재현은 안 됐다) — 라이브에서 실 인시던트가 **실제로 스코프를 얻고** 이웃 테넌트는 REFUSED. **교훈**: **생산자 없는 메커니즘은 테스트에서 영원히 초록이다**(스코프 17개 테스트가 전부 자기가 만든 레코드로 통과 — "게이트가 동작한다"만 증명하고 "무엇이 게이트를 여는가"는 안 물었다) · **내 가드가 나를 잡았고**(생산자만 생기고 자격증명이 없자 red) **그 가드도 심볼 하나만 봐서 새 생산자를 놓쳤다**(체인으로 교체) · 서명이 사는 건 **테넌트 바인딩**이지 **프로세스 내 코드 실행 방어가 아니다**. 그 앞은 **배포 네임스페이스 출처**(`6beebbc`, D37, gate 1565) — 행이 착지한 ns를 안 적어 **네 층이 각자 `"default"`를** 채웠고, 라이브에서 `rollout undo -n default`가 **엉뚱한 워크로드를 되돌리며 성공을 보고**했다. **티어(D36)와 달리 부재 보존이 아니라 해소된 사실 기록**이다. **D36이 세 번째·네 번째 경계에서 살아 있었다**(가드가 **열거**해서). **M13 전체(14건) → `COMPLETED_SUMMARY.md` M13. 교훈 7종**: 가드는 **호출부에서** 반증 · 픽스처는 **실제 입력**에서 · **소비자를 단언하라, 생산자 말고** · **투영/스키마 계층도 소비자**다 · 가드는 **파생시켜라, 열거하지 말고** · **생산자가 여럿이면 다수결이 결함을 가린다** · **수호 테스트 자신이 안티패턴일 수 있다**(4회). 반복 스윕 **양방향**: `scripts/find_unconsumed_fields.py` · `scripts/find_unwritten_keys.py`. 둘 다 **후보≠결함**.
>
