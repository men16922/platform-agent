...
- **검증:** `make check` → **2059 passed, 2 skipped · 32.35s** (**2026-08-15, 로컬 macOS·py3.13** — ⚠️**1929 이후는 로컬만**(미push), CI 일치는 1912까지; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M26**에 있다 — 여기에 다시 적지 말 것.
