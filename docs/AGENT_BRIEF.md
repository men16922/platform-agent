...
> ⚠️ 초록에는 조건이 붙는다(Risk 12): **새 가드는 지워 보고 red를 확인**(③) · **skip은 실패가 아니다** — 게이트 숫자엔 **잰 기계**까지 붙일 것(②) · **가드는 독자가 읽는 그 물건에 대고 물을 것**(④, 08-11 전수 훑기로 소진 — 다만 **결함을 그 그림자로 세지 말 것**: 잘못 문 가드는 3건인데 깨진 프로그램은 8건, 둘은 테스트에 이름조차 없었다. **경고는 주장만 묻지 말고 지시까지** 물을 것. 그리고 **문은 넷이다** — `print`·stderr 말고 **로깅**과 **잡히지 않은 예외**로도 리포트가 갈린다).
> 직전 세션 상세 → `COMPLETED_SUMMARY` **M17**(리포트 스트림 전수 훑기) · **M16**(비용 관측 3 provider, D44~D46).
>
...
- **검증:** `make check` → **1825 passed, 2 skipped** (**2026-08-12, 로컬 macOS·py3.13**; 직전 1789까지 CI가 다섯 지점 전부 숫자 동일 — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 이제 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M17**에 있다 — 여기에 다시 적지 말 것.
