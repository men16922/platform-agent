...
> ▶ NEXT SESSION: **Phase 2 잔여 2건** — ①**어댑터 helm values seam**(선언한 차트 대부분이 values 없이는 `helm template`조차 실패한다 — loki가 라이브에서 `bucketNames.chunks`를 요구했다. 테넌트별 실제 설치의 전제 조건) → ②faked managed 디스크립터(`applicable=false`)·DR 재구축 확인. 설계 `docs/plans/2026-07-21-multi-tenant-env-addons.md`(v5). **직전 완료**: 삭제 cascade(`e1ea15f`) · capability scope 축(`bb7a819`) · ⑥ NetworkPolicy 실제 활성화(`3dbc572`) · push 2축 수집기 + 대시보드 스위처(`b2b52fc`) — gate **1271**(미푸시 5커밋). kind에 acme-dev(4ns)·globex-dev(1ns) 테넌트 + NetworkPolicy 5개 가동 중. MLX는 내려둠.
>
