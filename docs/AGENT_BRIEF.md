...
> ▶ NEXT SESSION: **첫 행동 = 08-19 이후 AMP 실제 청구액 대조.** 4a는 **라이브**다
> (`docs/plans/2026-08-15-4a-remote-write-allowlist.md` **§9가 권위**) — AMP가 허용목록
> 4종 **308 시계열**을 그대로 반환한다(D50). ⛔**$1.42/월은 아직 산수다**: CE 2일 지연이라
> `aws ce get-cost-and-usage`로 AMP 라인을 확인해야 닫힌다 — ⚠️**크레딧 제외 필터 필수**
> (없으면 이 계정은 **$0으로 보인다**, 크레딧이 상쇄한다).
...
> 직전 세션 → **08-18**(`cost_metrics` 잔여를 측정으로 닫았다 — **결함이 아니라 범위**이고, 면제는 목록이 아니라 **경계**다: 가드가 의무를 읽는 쪽 선택 규칙에서 유도하고 전 모듈을 rglob으로 훑는다, 변이 3종 red · src 변경 0) · **M37**(티어 2가 **조건 평가 없이** 추천에서 액션을 만들어 **에스컬레이션 전용 capability 둘**이 first-response로 실행되고 있었다 — ⚠️**티어 2 액션을 단언하는 테스트가 0개**였다 · 관리형은 매니페스트를 안 내고 **read model이 부재를 설명한다**) · **M36**(ⓒ — 기존 가드가 **생산에서 도달 불가한 capability 집합**으로 물어 **첫-매치와 점수제의 답이 같은 경우만** 태웠다: GCP/Azure는 인시던트와 무관하게 늘 rto **180**을 보고했다, +12) · **M35**(signal↔capability 스윕이 **결함 넷** — `rebalance_consumer`가 **Azure만** 없는데 **같은 커밋에서 태어나 처음부터 빠져 있었다**, +6) · **M26**(렌즈가 `ExecutorOutput`에서 **말랐다**, 동작 변경 0) · **M25**(optional 임포트 **여섯이 미선언** — `.[azure]`엔 LLM조차 없었다, +19) · **M24**(게이트가 **테스트마다 Gemini를 과금 호출** — 288s→39s, D49) · **M23**(`Destroy`가 **AWS에만 없어** AWS에선 **AUTO**였다, +79) · **M22**("과거 유사 인시던트"가 **랜덤 ID 사전순** — 이번엔 **AWS만**, +13) · **M21**(`reason`이 규칙 이름을 잃어 **P3↔P1** 뒤집힘 — 형제가 provider가 아니라 **같은 provider의 진입점**, +17). 그 이전(M20~M18)은 `COMPLETED_SUMMARY`가 권위.
>
...
- **검증:** `make check` → **2285 passed, 2 skipped** (**2026-08-18 로컬 macOS·py3.13** — 08-17의 **2279**를 같은 기계에서 재측정해 동일함을 확인한 뒤 **+6**(py↔ts 상태 계약 가드, Phase 0 DoD의 ts 절반) · CI도 **같은 게이트**를 돈다(ubuntu·py3.13) — **CI 숫자는 PR 체크가 권위**, 여기 복제 금지; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M33**에 있다 — 여기에 다시 적지 말 것.
