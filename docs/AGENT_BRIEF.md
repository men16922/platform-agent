...
> 직전 세션 → **M21**(`reason`이 규칙 이름을 잃어 등급이 P3↔P1로 뒤집혔다 — 형제가 provider가 아니라 **같은 provider의 진입점**이었다, +17) · **M20**(운영자 severity가 두 모델에 안 닿았다, +18) · **M19**(`resource_types` 미판독, +32) · **M18**(형제 집합, D47).
>
...
- **검증:** `make check` → **1929 passed, 2 skipped** (**2026-08-15, 로컬 macOS·py3.13** — ⚠️**1929는 아직 로컬만**(미push), CI 일치는 1912까지 확인됨; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M21**에 있다 — 여기에 다시 적지 말 것.
