...
> ⚠️ **"안 봤다"를 시험하면 결함이 아니라 범위가 나올 때도 있는데, 그 과정에서 절반만 묻던 가드가 나온다**(08-13에 두 번). 그리고 ⚠️**형제 집합은 세는 순간 전부 셀 것** — 하루에 세 번 걸렸고 **세 번째는 그걸 찾으려고 만든 스윕 안에서** 밟았다(Risk 12⑥).
> 직전 세션 → `PROGRESS_LOG` 상단 3건(형제 집합 스윕 · 로깅 문의 거울 의무 · GCP/Azure capability 스캔) · `COMPLETED_SUMMARY` **M17**(리포트 스트림) · **M16**(비용 관측, D44~D46).
>
...
- **검증:** `make check` → **1862 passed, 2 skipped** (**2026-08-13, 로컬 macOS·py3.13**; 1789까지 CI가 다섯 지점 전부 숫자 동일 — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M17**에 있다 — 여기에 다시 적지 말 것.
