...
> ▶ NEXT SESSION: **차단 없는 잔여는 소진됐다(2026-07-29, 읽기 모델 드리프트 + 문서 정리까지). 남은 것은 결정 3건 + 승인 1건(실 DynamoDB 왕복 검증 — 실 AWS 자원 필요).** ①**"배포는 어느 테넌트 소유인가"가 두 항목을 동시에 막고 있다** — deployments/activities 파티션은 물론 **모델 호출 rate limit도** 여기 묶여 있다(로컬 모델 호출자는 `local_deployer`/`strands_deployer` 둘뿐, 둘 다 배포 경로. 배포 요청엔 테넌트가 없고 `setup_tenancy`는 **모델이 부르는 도구** — 테넌트가 추론의 **입력이 아니라 출력**이라 신원 전파 자체가 성립 안 함. 발명 금지) · ②**무스코프 MCP 읽기가 여전히 ambient**(검증된 익명 kagent 왕복을 살리려는 의도적 예외 — 닫으려면 kagent 경로에 스코프 선행) · ③**Capsule `limitRanges` 이관 경로 결정**(`GlobalTenantResource`=클러스터 스코프라 **D30 위반**, `TenantResource`=SA+RBAC 새 권한 표면). 그 다음은 **Phase 4**(managed, billable) 또는 **Phase 5**(레지스트리 쓰기 → D32 재검토 조건). **주의**: 카드/스키마/문서에 **집행하지 않는 것을 광고하지 말 것** · 가드를 쓰면 **호출부에서 반증할 것**(이번에 새 테스트 20건이 전부 통과하는데 호출부만 되돌아간 상태였다 — 라이브만 잡았다) · 푸시/네트워크 신규 필드는 항상 optional+폴백 · 대시보드는 `dashboard/AGENTS.md`대로 **Next 문서 먼저** · 자격증명이 테넌트-바운드인 것은 **온프렘뿐**(D31) · 파티션된 읽기 경로는 **둘뿐**(플릿·인시던트). **직전 완료**: **읽기 모델 문서 드리프트**(`61ee2f4`, gate **1533**) — M13이 **한 층 위로** 열렸다: 필드가 아니라 **선언 자체를 아무도 안 읽는** 경우. `activity-model.ts`는 **importer가 0**이라 양방향으로 어긋난 채였고(아무도 안 쓰는 `duration_ms` 선언 · 상세 페이지가 딛고 선 `trace`·`cost_metrics`·`deployment_id` 누락), **거짓 주장 둘**(`ttl` "30일 보관"인데 주 writer가 안 써 **만료 안 됨** · `GSI1`은 절반만 채워지고 무쿼리라 그대로 쿼리를 짰으면 **조용히 짧은 목록**). 지키던 테스트가 **부분문자열 존재만** 봤고, **내가 새로 쓴 가드도 처음엔 `any`를 쓸 자리에 `all`이 필요한 같은 병**이었다(반증에서 잡음). 그 앞은 **롤백 비용 패널**(`db41874`, gate 1528) — M13이 **반대 방향으로** 열렸다. 앞의 아홉은 "선언됐는데 아무도 안 읽음", 이번은 **"읽는데 아무도 안 씀"**: ACTIVITY를 쓰는 셋 중 `record_rollback`만 `cost_metrics`를 빠뜨렸고, 읽는 쪽 `mergeActivity`가 **trace만 합집합**으로 두고 나머지를 `{...latest}`로 최신 행에서 가져가 **롤백되는 순간 도구/추론/토큰 수가 페이지에서 사라졌다**(패널이 조건부라 예외도 "0"도 없이). 라이브 BEFORE 미렌더 → AFTER `tool calls 5 · tokens 920`. **교훈**: 절반씩은 각각 방어 가능한데 **겹칠 때만** 터지는 결함이 있다 · 생산자가 여럿이면 **하나만 침묵해도** 나머지가 정상을 계속 증명해준다. 새 스윕 `scripts/find_unwritten_keys.py`(대시보드가 읽는데 생산자 없는 키 — 기존 스윕의 반대 방향). 그 앞은 **time-to-resolve**(`3a89e43`, gate 1520) — M13의 아홉 번째이자 가장 비싼 변종. 앞의 여덟은 값이 **버려졌지만** 이번엔 값이 **있는 척했다**: `resolved_at`이 `created_at`의 복사본이라 주간 온콜 리포트가 **존재 내내 "MTTR 0.0분"을 자신 있게 발송**했다(실측 0.0→45.0, 라이브 1502초 보존). 같이 나온 둘: fetch가 `runbook_id`에 `alarm_name`을 복사해 재발 패턴 그룹핑이 붕괴 · 대시보드 Scan 투영이 **자기 리더가 읽는 4필드**(`triggered_at`·`confidence`·`reconciliation`·`trace_id`)를 안 불러 아침 수정이 배지 한 층 앞에서 멈춰 있었다. 그 앞은 **M13 = 총 11건** → `COMPLETED_SUMMARY.md` M13. **교훈 7종**: 가드는 **호출부에서** 반증 · 픽스처는 **실제 입력**에서 · **소비자를 단언하라, 생산자 말고** · **투영/스키마 계층도 소비자**다(`ProjectionExpression`이 안 부른 필드는 리더가 아무리 잘 짜여도 복구 불가) · 가드는 **파생시켜라, 열거하지 말고**(매퍼가 읽는 속성을 파싱해 요구 → 다음 필드에도 실패) · **생산자가 여럿이면 다수결이 결함을 가린다**(셋 중 하나만 침묵하면 나머지 둘이 정상을 계속 증명해준다 — 소비자를 단언해도 안 잡힌다) · **수호 테스트 자신이 안티패턴일 수 있다**(부분문자열 존재만 보는 테스트는 모양을 못 본다 — 내 새 가드도 처음엔 `any`/`all`을 틀렸다). 반복 스윕 **양방향**: `scripts/find_unconsumed_fields.py`(선언됐는데 안 읽힘) · `scripts/find_unwritten_keys.py`(읽는데 생산자 없음). 둘 다 **후보≠결함**.
>
