...
> ⚠️ 초록에는 조건이 붙는다(Risk 12): **새 가드는 지워 보고 red를 확인**(③) · **skip은 실패가 아니다** — 게이트 숫자엔 **잰 기계**까지 붙일 것(②) · **가드는 독자가 읽는 그 물건에 대고 물을 것**(④, 08-11 전수 훑기로 소진 — 다만 **결함을 그 그림자로 세지 말 것**: 잘못 문 가드는 3건인데 깨진 프로그램은 8건, 둘은 테스트에 이름조차 없었다. 그리고 **경고는 주장만 묻지 말고 지시까지** 물을 것).
...
- **검증:** `make check` → **1779 passed, 1 skipped** (**2026-08-11, 로컬 macOS·py3.13**; CI가 1769·1773 두 지점에서 숫자까지 일치 — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 이제 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M17**에 있다 — 여기에 다시 적지 말 것.
