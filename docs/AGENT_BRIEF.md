...
> **BQ 결제 내보내기**(콘솔 수동, 사용자 몫) — 없으면 경보가 울려도 **금액을 모른다**.
...
- **검증:** `make check` → **1708 passed, 1 skipped** (**2026-08-09, 로컬 macOS·Python 3.13 — CI 미확인** — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 이제 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M15**에 있다 — 여기에 다시 적지 말 것.
