...
> 필요) · "런북 walk가 남긴 ②"(`severity` 축) · 리포트 스트림 REPORT 4개.
>
...
> 직전 세션 → `PROGRESS_LOG` 상단 2건(GCP/Azure capability 스캔 · 비용 전수 측정) · `COMPLETED_SUMMARY` **M17**(리포트 스트림) · **M16**(비용 관측, D44~D46).
>
...
- **검증:** `make check` → **1856 passed, 2 skipped** (**2026-08-13, 로컬 macOS·py3.13**; 1789까지 CI가 다섯 지점 전부 숫자 동일 — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M17**에 있다 — 여기에 다시 적지 말 것.
