...
> ▶ NEXT SESSION: `docs/NEXT_PLAN.md` — **레퍼런스 Tier 2 #2 완료**(agents-as-tools 오케스트레이션 + self-consistency 라우팅, 신규 `orchestration.py`, a2a_server 옵트인·비파괴, +12 test, gate **714**). Tier 1도 완료(reconciliation·비용 게이트·서킷브레이커·비용 서브메트릭). 클라우드 라이브 전부 완료(Runtime 3/3·Provision GKE/AKS)·Vercel 대시보드 공개 안정화. **다음 우선순위 = Tier 2 잔여**: #3 MCP-over-HTTP 커넥터+per-tool kill-switch(앵커 `gateway/mcp_server.py` `TOOL_CATALOG`)·#4 cross-account STS AssumeRole+fallback(AWS 어댑터, 규모 작음) — 각 새 세션 권장. 외부: Slack App·아티클. 하네스: billable create·보안완화는 사용자 `!` 필요.
>
