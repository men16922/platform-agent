...
> ⚠️**그보다 먼저 $0·무승인 선행이 둘**(둘 다 사용자 몫): ①계정 전체 예산 `Smart-EV demo
> budget 20USD`가 **₩20으로 설정돼 상시 발화** 중이라(이름과 1,400배 차이) Phase 4 경보도 같이
> 묻힌다 ②**사후 조회가 없다**(Billing API 비활성 + BQ 내보내기 미완) — 07월 GKE 방치 비용을
> **울렸어도 얼마인지 모른다**. 임계 경보는 **있다**(초판의 "관측 0"은 잘라 읽은 오판, 2026-08-09 정정).
>
...
- **검증:** `make check` → **1697 passed, 1 skipped** (**2026-08-09 측정, 로컬·CI 일치** — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 이제 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M15**에 있다 — 여기에 다시 적지 말 것.
