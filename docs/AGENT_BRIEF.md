...
> 직전 세션 → **M20**(운영자 severity가 두 모델에 안 닿았다 — PR #37, +18) · **M19**(`resource_types` 미판독 — PR #33·#34, +32) · **M18**(형제 집합, D47) · **M17**(리포트 스트림).
>
...
- **검증:** `make check` → **1912 passed, 2 skipped** (**2026-08-15, 로컬 macOS·py3.13 · CI도 1912 — 숫자 일치**; 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M20**에 있다 — 여기에 다시 적지 말 것.
