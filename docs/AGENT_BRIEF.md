...
- **검증:** `make check` → **1773 passed, 1 skipped** (**2026-08-11, 로컬 macOS·py3.13**; CI 일치는 1769 지점에서 확인 — 게이트 숫자는 날짜와 **잰 기계** 없이는 주장이 아니다, Risk 12①②). CI(`gate.yml`)가 **main 병합 조건**이고 이제 **terraform 검증까지 실제로 돈다**. 이력·근거는 `STATUS` 검증 Baseline과 `COMPLETED_SUMMARY` **M10~M17**에 있다 — 여기에 다시 적지 말 것.
