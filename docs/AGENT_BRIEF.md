...
> ▶ NEXT SESSION: **①Phase 3② 롤백↔selfHeal 우선순위**(registry write-back, 설계 `docs/plans/2026-07-21-multi-tenant-env-addons.md` 289–290행 · 지금은 문서로만 명명돼 있고 `ONPREM_EXECUTOR_LIVE=false`로 묶여 있다) → **②Phase 3③ viewer 가시성**. **발행 3종 전부 완료**(Notion 전문 · YouTube Shorts `2J9WfZV0TPE` · LinkedIn, 2026-07-28) — 남은 건 **레포 원고를 게시본에 맞추는 것**뿐이다(`docs/post/linkedin-intro-ko.md` 34·71행 "7B가 30B를 이겼습니다"). **주의**: 푸시 신규 필드는 항상 optional+폴백(`tsc`는 구버전 페이로드 런타임 오류를 못 잡는다) · 새 애드온은 PSS를 `--dry-run=server`로 먼저 확인 · 새 백엔드는 `self_hosted_repo`도 함께 · **에이전트의 mutating 범위는 테넌트 스코프까지**(공유 스택 9개는 TF 소유, D30) · **자격증명이 테넌트-바운드인 것은 온프렘뿐**(GCP=프로젝트 전역, Azure=cluster-admin kubeconfig → Phase 4). **직전 완료**: Phase 3① 자격증명 격리 full + `render_rbac`의 바인딩 대상 SA 부재 근본수정 — gate **1355**, `main` origin 동기화됨(Phase 3① 커밋 `bb091e1`은 미푸시).
>
...
- **현재 초점:** **Phase 3(인가 강화)** — ①자격증명 격리 full 완료, 다음은 ②롤백↔selfHeal 우선순위 · ③viewer 가시성. 발행 3종(Notion 전문 · YouTube Shorts · LinkedIn)은 **전부 완료**(2026-07-28), 레포 원고 동기화만 잔여. **자연어 한 문장이 테넌트를 세운다** — 에이전트 mutating 범위는 테넌트 스코프까지고 공유 스택 9개는 TF 소유(D30). 시연은 `make dev-up` → `make demo-baseline` 두 줄로 재현되고, 격리 반증(netpol 1개 삭제 → network 축만 ✕ → 복구)까지 실증됐다. 대시보드가 멀티테넌시를 관제하고(플릿 표+격리 4축), 검증은 훅으로 강제된다(Stop→make check, PostToolUse→tsc). Phase 2 완결 — 소프트 티어 격리가 네 층(네임스페이스·쿼터·데이터플레인·자격증명)에서 라이브 실측됨. 소프트 티어는 이제 네임스페이스+쿼터(Capsule)에 더해 **데이터플레인 격리**까지 실측됐고(same 통과/cross 차단/kubelet·DNS 무영향), 2축 상태는 각 클러스터의 **push**로 모인다(허브 read 자격증명 0). Phase 1b rollouts-demo 이관 완료(stateful 3건은 스냅샷 선행). 런북은 **선언한 순서·조건·검증대로 실행**된다. 관측성은 metrics+logs+**traces** 삼각(OTel→Tempo). 과금 가드 3중(모두 report-only). 공급망: PSS restricted 기본 ON + Cosign 검증 게이트(어드미션 집행은 미도입, 의도적).
