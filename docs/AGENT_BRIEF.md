...
> ▶ NEXT SESSION: **Phase 2 잔여 3건** — ①**클러스터 싱글턴 capability의 scope 축**(라이브에서 컨트롤러 2개가 같은 Rollout을 조정했다 → 카탈로그에 `scope: cluster|namespace`, 어댑터가 테넌트별 렌더 거부. **그전까지 어댑터 렌더 결과를 그대로 적용하지 말 것**) → ②ArgoCD Application 삭제 시 워크로드 고아(파이널라이저) → ③faked managed 디스크립터(`applicable=false`)·DR 재구축 확인. 설계 `docs/plans/2026-07-21-multi-tenant-env-addons.md`(v5). **직전 완료**: ⑥ NetworkPolicy 실제 활성화(`3dbc572`) · push 기반 2축 drift 수집기 + 대시보드 tenant/env 스위처(`b2b52fc`) · Phase 2 첫 슬라이스 tenancy+Capsule — gate **1251**(미푸시 2커밋). kind에 acme-dev(4ns)·globex-dev(1ns) 테넌트 + NetworkPolicy 5개 가동 중. MLX는 내려둠.
>
