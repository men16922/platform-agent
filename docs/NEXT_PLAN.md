...
## 현재 상태 (2026-08-11, gate 1787 — 로컬 macOS·py3.13)
...
- [ ] **(별도 계획) GitAIOps 후속편 아티클** — 논지=책은 AI 자리에 사람이 프롬프트를 넣지만
  ...
  **새 소재**: 소비자 없는 선언 14건(M13) · **수호 테스트 자신이 같은 안티패턴**(4회) ·
  **생산자가 테스트뿐인 메커니즘**(결정 5) · **리포트가 독자에게 갈린 채 도착**(M17, 문 넷).
...
- **2차 잔여 = 하나**: **스포크의 읽기 신원** — `_kubectl`이 맨 kubectl이고 공유 `argocd` ns를
  읽어 테넌트 구분이 **코드 필터**다(쓰기는 401로 막지만 읽기는 안 막는다). **시끄럽게만** 해
  뒀고(`warn_if_ambient_read` — 08-11에 그 경고가 **stderr로 새고 있던 것**을 고쳤다, M17),
  seam은 **인클러스터 배포·민팅이 선행**이라 인프라 결정. 증거 `push-identity-ambient.log`.
...
- [ ] **`slack_live_approval.py` 이중 노후화 — 고치면 조용히 no-op이라 안 고쳤다(08-11 실측)**
  ①임포트 `src.agents.operations.approval_bridge`는 **untracked `cdk.out`에만** 있다(추적본은
  `...operations.aws.approval_bridge` — "`cdk.out`이 grep에 잡힌다" 함정 재발) ②고쳐도 덮어쓰는
  여섯 이름 중 **넷이 부재** → **돌아가면서 아무것도 안 한다**. 올바른 이름은
  `slack_interactive`/`request_store`로 보이나 **데모를 Slack에 태워야 확정**된다.
- [ ] **리포트 스트림: 못 한 것과 안 한 것(2026-08-11, 상세는 증거 로그 13절)** — 파이프 뒤는
  **11 invocation / 9 CLI**. 나머지는 라이브 자격증명·기동한 스택이 필요하거나 **강제할 실패
  경로가 없다**; 그걸 요구하는 가드는 **skip되는 가드**라 Risk 12②를 새로 만든다.
  ⚠️`live_tier2_demo`는 스택 없이 돌리면 **본문 stdout + 트레이스백 stderr**로 같은 결함인데
  고치려면 **검증 수단이 선행**. 로깅 문은 **REPORT 4개만**(DOCUMENT/DUAL은 의무가 거꾸로).
...
- [ ] **GCP/Azure 90일 보관 = 승인 항목이 아니었다** — **`STATUS` Risk 2가 권위**. 스토어가
  아예 없어 **지울 데이터 0**, 보관하려면 **먼저 프로비저닝**(billable) → Phase 4. 코드 갭 유효.
...
- [ ] **MCP 게이트웨이는 여전히 포트에 붙어 있지 않다** — 결론 유지, **근거는 틀렸다(재측정)**:
  `bridge.py:35`에 기본값 폴백이 있고 그 `McpA2aBridge`를 만드는 건 **테스트뿐**이다.
  ⚠️**세는 함정 둘** — `cdk.out`은 untracked인데 파일 grep엔 잡힌다(→ **`git grep`**, 08-11에
  `slack_live_approval`에서 재발) · **docstring 예시**가 호출로 보인다. 붙일 땐 **요청 경로가
  스코프를 공급하는지** 확인 — D39가 닫은 건 무스코프 읽기일 뿐 **"강제된다"는 아니다**.
