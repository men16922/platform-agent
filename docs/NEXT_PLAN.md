...
(08-09 두 번 · 08-10~13 · **08-15**). **소진은 목록의 상태지 사실이 아니다** — 목록 밖에도
있고, **가장 값싼 다음 수는 대개 직전 세션이 이미 적어 뒀다**. ⚠️**08-15**: **목록에 적힌
항목 자체가 틀릴 수도 있고**(→ M19의 ⓑ) **그래도 시험하면 값이 난다** — 단 **`git log -L`로
"언제부터"까지** 물어야 stale과 오기를 가른다.
...
- [ ] **(별도 계획) GitAIOps 후속편 아티클** — 논지=우리는 **오프라인 Qwen 에이전트로 루프를
  ...
  안티패턴**(7회+, M17~M19가 그 사례집). **집필·발행은 착수하지 않는다**(지시 2026-07-25).
...
- **2차 잔여 = 하나**: **스포크의 읽기 신원** — 맨 kubectl로 공유 `argocd` ns를 읽어 테넌트
  ...
  (`warn_if_ambient_read`, M17 · `push-identity-ambient.log`), seam은 **인클러스터 배포·민팅이
  선행**이라 인프라 결정.
...
- [ ] **`slack_live_approval.py` 이중 노후화 — 고치면 조용히 no-op이라 안 고쳤다(08-11)**
  ...
  → **돌아가면서 아무것도 안 한다**. 올바른 이름은 **데모를 Slack에 태워야 확정**된다. 리포트
  스트림의 나머지는 08-13에 닫혔다(M18). ⚠️**안 만든 것**: DUAL의 **모드 조건부 리다이렉트**
  — 둘 다 경고에 못 닿아 **하중을 못 받는 가드**가 된다.
...
- [ ] **capability 스캔이 남긴 셋(08-13) — ⓑ는 08-15에 닫혔다, ⓐ·ⓒ는 정책 사안이라 안 고쳤다**
  ...
  경위와 그 자리에서 나온 진짜 결함은 **M19가 권위** · ⓒ**티어 2는 첫 매치가 이긴다**(AWS는
  점수제) — 테스트가 고정했으니 **우연이 아니라 결정**이다.
...
- [ ] **GCP/Azure 90일 보관** — **`STATUS` Risk 2가 권위**(스토어가 없어 지울 데이터 0 →
  Phase 4). **기록기의 `resolved_at`·`triggered_at`도 읽는 쪽이 없다** · **analyzer LLM 폴백은
  일괄 P2**로, severity 매핑은 **정책 결정**이라 발명 안 함.
- [ ] **k3s 승격은 닫혔다(D40) — 여는 조건은 "k3s-lab에 워크로드"** — 경위는 **`STATUS` Risk 5**가
  권위. 경로는 **옵션 A**(네임스페이스·netpol만, 비용 0) — ⚠️**실체 없는 선언 2건**을 더하므로
  그때 재평가.
...
이전). **기준은 읽는 쪽의 provider 간 비대칭**이지 선언의 고아 상태가 아니다.
