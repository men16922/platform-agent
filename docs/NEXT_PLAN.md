...
- [ ] **Phase 4 남은 것 = 4b**(자격증명 파티션 미증명=Risk 10 · 30분 TTL 가드가 상시와 안 맞는다, billable·별 승인) ·
  ✅**$0 선행 BQ 결제 내보내기는 09-01에 켰다** — 표준 사용량 비용 `사용 설정됨`(`project-ec7809f7…:billing_export`). ⛔**아직 "잴 수 있다"가 아니다**: `spend-check`는 여전히 *"아직 못 잰다"*이고 **그건 `GCP_BILLING_EXPORT_SETUP.md` §3이 예고한 정상 상태**(테이블까지 수 시간). **다음 세션의 첫 확인이 그 줄이다** — 하루 뒤에도 그대로면 그때가 문제.
