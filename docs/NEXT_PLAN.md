...
- ⛔**Phase 4 / 4a는 닫혔고(08-30, M38) 접혔다(2026-09-01, 사용자 결정 — M40 · 계획서 §11 · D50 Folded).** 실제 청구 **$0.00**(프리티어 `Always Free` 40M). 워크스페이스 `ws-929b8da9…`·IAM 사용자·키 **셋 다 삭제**, 8리전 스윕 0으로 확인. ⚠️**$0.00이 접은 이유가 아니다** — 대가는 **장기 키**였고 D50이 미리 그렇게 적어 뒀다. ⚠️**D48은 안 죽었다**(허용목록 없이 remote_write 금지): 필터 없음 **128배 초과**는 그대로라 `test_amp_cost_handles`를 **지우지 않고** 계약을 함수로 빼 **합성 표**에 물렸다(+7, 변이 7 red). ⚠️**`AccessDenied`를 부재의 증거로 읽지 말 것** — 증거는 **소유 사용자의 부재**다. **다시 열지 말 것.** ✅**남은 손질도 09-01에 끝났다**: kind를 띄우니 **삭제된 워크스페이스로 403이 계속 나오고 있었다**(예고대로) — **값 파일이 아니라 배포된 릴리스가 옛것이었다**. helm rev 5→6 + 고아 Secret 삭제 + 런타임 API로 확인(remote_write 0건, 오류 0건). ⚠️**`--reuse-values`가 그 블록을 되살린다**(dry-run이 잡았다).
...
⛔**정적검사는 게이트에 안 들어간다 — 닫혔다(2026-09-01, 사용자). `DECISIONS` D51이 권위.** ⚠️**"나중에 보자"가 아니라 측정 위에서 내린 결정이다**: ruff는 **20건 전부 미관**(여섯을 직접 열어 확인, **결함 0**) ⇒ 넣어도 **스타일 집행이지 버그 차단이 아니다** · mypy는 **253 중 200이 주석 부채, 53이 주장**이고 그 53 중 **21건을 열어 고칠 값이 있던 건 2건**(M44·M45)인데 **`mypy src/`가 45초**로 게이트(38초)를 두 배로 만든다(**D49와 정면 충돌**). ⚠️**"정적검사는 쓸모없다"로 요약 금지** — 이번 세션의 **실제 결함 둘을 찾은 건 mypy이고 테스트 스위트는 둘 다 놓쳤다**. 결정된 건 *가치*가 아니라 **상주시킬 값어치가 아직 아니라는 것**이다. **여는 조건은 D51에.**
