...
## 현재 상태 (2026-08-15, gate 1894 — 로컬 macOS·py3.13 · CI 일치)
...
**남은 건 Phase 4(billable, 별 승인)뿐** — 단 "무과금 작업 소진"은 **이제 여섯 번 틀렸다**
(08-09 두 번 · 08-10 · 08-11 · 08-12 · 08-13 · **08-15**). **소진은 목록의 상태지 사실이
아니다** — 목록 밖에도 있고, 목록이 **무엇의 그림자인지**도 물어야 하고, **가장 값싼 다음
수는 대개 직전 세션이 이미 적어 뒀다**. ⚠️**08-15에 하나 더**: **목록에 적힌 항목 자체가
틀릴 수 있다**(ⓑ는 "매핑이 없다"였는데 네 provider 전부 있었고 **기록보다 한 달 먼저**
있었다) — 그런데 **그걸 시험하는 과정에서 옆의 진짜 결함이 나왔다**. **틀린 항목도 값이
난다. 단 `git log -L`로 "언제부터"까지 물어야 stale과 오기를 가른다.**
...
- [ ] **(별도 계획) GitAIOps 후속편 아티클** — 논지=우리는 **오프라인 Qwen 에이전트로 루프를
  ...
  안티패턴**(7회+) · **리포트가 독자에게 갈린 채 도착**(M17) · **두 provider에 복사된 죽은
  티어**(M18) · **선언은 됐는데 두 provider만 안 읽던 계약 필드**(M19). **집필·발행은
  착수하지 않는다**(지시 2026-07-25).
...
- [ ] **Phase 5 = 경계까지 섰다(2026-08-08, → M15)** — UI가 아니라 **"PR은 그 테넌트 파일
  ...
  별개 잔여가 **아니라 이 항목의 구속 조건**. 우회 없음(OAuth `repo`는 blast radius=1과 충돌 ·
  TS 이식은 **두 번째 진실 공급원**). ⇒ 여는 조건 = **파이썬 플래너를 어디서 돌릴지**.
- [ ] **Phase 4 — 산정 끝, 승인 대기.** 근거는 **`docs/plans/2026-08-08-phase4-scope-and-cost.md`가
  ...
  **`docs/GCP_BILLING_EXPORT_SETUP.md`**(콘솔 수동, 사용자 몫 · 확인은 `make spend-check`).
  ...
  Phase 5가 열리면 3②를 GitOps-native로(D32) · **런북 스토어도 여기서 처음 생긴다** — 등록
  문서의 RTO 필드는 **`rto_sec`**이다(**D47**, 지금은 문서 0개라 마이그레이션 없음).
...
- [ ] **`slack_live_approval.py` 이중 노후화 — 고치면 조용히 no-op이라 안 고쳤다(08-11)**
  ...
  → **돌아가면서 아무것도 안 한다**. 올바른 이름은 **데모를 Slack에 태워야 확정**된다. 나머지는
  08-13에 닫혔다(증거 15절: 변명은
  **참**이었고 **대신 가드가 절반만 묻고 있었다** → 셋 추가, 변이 5 red). ⚠️**안 만든 것**:
  DUAL의 **모드 조건부 리다이렉트** — 둘 다 경고에 못 닿아 **하중을 못 받는 가드**가 된다.
- [ ] **런북 walk가 남긴 ②(08-12)** — ①은 **08-13에 닫혔다**(→ `PROGRESS_LOG` 상단). 조건 축은
  ...
  하중을 못 받는다).
- [ ] **capability 스캔이 남긴 셋(08-13) — ⓑ는 08-15에 닫혔다, ⓐ·ⓒ는 정책 사안이라 안 고쳤다**
  ⓐ**`kafka-lag-spike`만 두 dict가 어긋난다**: 스텝엔 `rebalance_consumer`가 있는데
  `capabilities`엔 없다(9개 중 이것 하나, **어긋난 쪽이 또 에스컬레이션 스텝**) — 더하면
  매치 면이 넓어지고 빼면 스텝이 준다, **둘 다 동작 변경** · ~~ⓑ~~ **틀린 기록이었다**:
  매핑은 **네 provider 전부에 2026-07-09부터** 있어 그 기록보다 **한 달 앞선다**(stale이 아니라
  오기). 그 자리의 진짜 결함은 **티어 2가 `resource_types`를 안 읽는 것**이었고 고쳤다
  (→ `resource-types-declared-and-unread.log`) · ⓒ**티어 2는 첫 매치가 이긴다**(AWS는 점수제)
  — 테스트가 고정했으니 **우연이 아니라 결정**이다.
- [ ] **`record_route_activity`·`record_agent_activity`의 `cost_metrics` — 의도적으로 남김**.
  둘 다 `deployment_id`가 없어 그 필드를 렌더하는 유일한 뷰에 닿지 않아 **소비자 없는 필드**다
  (`activity-model.ts`가 `deployment_id`로 접는다 — 08-08 재확인).
- [ ] **GCP/Azure 90일 보관** — **`STATUS` Risk 2가 권위**. 스토어가 없어 **지울 데이터 0** →
  Phase 4. 코드 갭은 유효. **기록기의 `resolved_at`·`triggered_at`도 읽는 쪽이 없다**(같은 곳).
  **analyzer LLM 폴백은 일괄 P2** — severity 매핑은 **정책 결정**이라 발명 안 함.
...
간 일치를 묻는 가드**로 묶을 것(`test_capability_catalog_scan.py`). 같은 이유로, **의무가
분류마다 뒤집히는 규칙은 양쪽을 다 물을 것**(`test_report_streams.py`의 REPORT↔DOCUMENT).
계약 규칙 자체는 **계약 모듈에 한 벌만** 둔다(08-15: `runbooks/schema.py::fits_resource`를
세 provider가 읽는다 — 복사본 둘은 다음 고침이 한쪽에만 닿는 방식이다).
⚠️**"선언됐는데 안 읽힌다"는 자동으로 결함이 아니다(08-15)** — `provider`는 **AWS 포함 아무
decision 경로도 안 읽는데** 빌트인 9개가 **전부 `"aws"`**라, 읽기 시작하면 GCP/Azure는 **전부
`generic-recovery`로 떨어진다**(=#30 이전). `actions`를 안 읽는 것도 **옳다**. **기준은 읽는
쪽의 provider 간 비대칭**이지 선언의 고아 상태가 아니다.
