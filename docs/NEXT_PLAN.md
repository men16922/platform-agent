...
- [~] **⑧ A2A 위임 계약 하드닝 (Tier 3)** — **안전 서브셋 완료(2026-07-17, gate 790→795)**: 아웃바운드 `sanitize_instruction`(control-char strip·4000자 cap·trace, 분류는 원문 유지)+`handle` 배선, 비파괴 +5 test. **잔여=승인 대기** (설계=`docs/plans/a2a-delegation-hardening.md`): ⑧-1 구조화 `{task_type,params}` 디스크립터(비파괴 증분) · ⑧-2 저-confidence 폴백→게이트(옵트인 DI) · ⑧-3 최소권한 힌트(role별 allowedActions, `action_sink_grader` 정책 공유=가장 안전) · ⑧-4 TOOL→SKILL→SUBAGENT smell-test 문서화. 자유텍스트 완전 제거·특화 서버측 시스템프롬프트 고정은 특화 서버 재작성 동반=별도 마일스톤.
