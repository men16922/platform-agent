...
## 현재 상태 (2026-08-30, gate 2299 — 로컬 macOS·py3.13 · CI도 같은 게이트, ubuntu·py3.13)
...
- [ ] **Phase 4 — 4a는 라이브다(08-17, D50). 남은 건 측정 하나와 설계 하나.**
  ...
  ✅**측정 닫힘(08-30) → 4a 종료. 실제 청구 $0.00**(계량 798,331 샘플 · 프리티어 **`Always Free` 40M/월** — §3의
  배제 근거는 **전건이 거짓**). ⚠️**절벽이 요율→한도로 옮겨 앉았을 뿐**: 필터 없음 **128배 초과**, 허용목록 하중 그대로 ·
  ⚠️파이프는 **연속이 아니다**(4일·41.3h, 08-27T19:55Z 이후 죽음) → $1.42는 720h 가정, 실측분 **$0.07**.
  권위 §10 · `docs/evidence/amp-actual-bill-is-zero-and-the-free-tier-reason-was-inverted.log`. **남은 건 4b**
  (자격증명 파티션 미증명=Risk 10 · 30분 TTL 가드가 상시와 안 맞는다) · ⚠️**$0 선행: BQ 결제 내보내기**(콘솔 수동 ·
  `make spend-check`) · **접으면 워크스페이스·IAM 사용자·키 셋 다 지울 것**(D50) — ⚠️**$0이라고 이 이유는 안 줄어든다**.
