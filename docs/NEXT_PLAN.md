...
8. [~] **⑦ 라이브 모델 스윕** — **어댑터 코드 완료(gate 819→822)**: `live_router_factory(call_model, backstop=)`(모델 응답→role 파싱·latency 측정·미파싱/실패 시 결정론 백스톱), `run_sweep`에 드롭인. **잔여=실 실행만**(사용자): 실 `call_model`(creds) 주입 + 과금 승인. 20케이스×N모델 = 저비용(짧은 분류 호출)이나 실 spend + 크레덴셜 필요. +3 test.
