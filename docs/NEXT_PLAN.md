...
8. [x] ~~**⑦ 라이브 모델 스윕**~~ — **완료(gate 822→823, 2026-07-17, 로컬 MLX spend $0)**: `scripts/live_model_sweep.py`로 160 라이브 호출(2모델×2 effort×20케이스×프롬프트 v1/v2). 라이브가 `_classify_prompt` teardown/진단동사 결함 표면화→v2 수정→회귀 가드. **증거 기반 선택: Qwen2.5-Coder-7B @ temp0 = 20/20·최속**(30B 반증). 증거 `docs/evidence/model-sweep-live.log`.

**→ 승인된 실행 큐 8항목 전부 완료(코드+실행). 잔여 백로그 = 전부 사용자/인프라.**
...
- [x] ~~**잔여 레퍼런스 #7(Helm/Terraform 프로덕션, Tier 3)**~~ — **#7-a Helm 파트 완료(2026-07-17, gate 823→829)**: `infra/helm/platform-agent/`(webhook+opt-in router·최소권한 RBAC·drain 별도 게이트·단일-writer PVC·kind/k3s values) + `infra/onprem/Dockerfile`(실빌드 881MB+컨테이너 스모크) + 가드 6종. 부산물: pyproject optional-deps PEP 621 latent 버그 수정. **kind 라이브 실증 완료(동일자, 승인)**: install→Ready→RBAC can-i 분리→Day-2 E2E(P2 승인 루프)→PVC 영속→teardown(`docs/evidence/helm-kind-live-install.log`). **#7-b Terraform 모듈 완료(동일자, gate 834→839)**: `infra/terraform/aws-production/`(VPC/EKS/Aurora `platform_state`=④ seam 정합/IRSA 정확-ARN grant), init+fmt+validate Success·apply=사용자 게이트, 가드 +5. **→ #7 전체 완결**. **차트 stateStore values 배선 + k3s substrate 스모크 완료(동일자, gate 842)** — kind/k3s 양축 실증(`docs/evidence/helm-k3s-substrate-smoke.log`). 잔여: (billable) terraform apply만.
...
- [x] ~~**⑦ 모델/파라미터 스윕 → Model Router 정량화 (Tier 2)**~~ — **전부 완료(2026-07-17, gate 790 스캐폴드 → 822 어댑터 → 823 라이브)**: 오프라인 스캐폴드+`live_router_factory`+**실 실행(로컬 MLX $0, 160콜)**. 정적주석→증거기반 선택 달성(7B@temp0=20/20 최속, 30B 반증) + 라이브가 프롬프트 결함 발견→수정→가드. 상세=★ 큐 8번·`docs/evidence/model-sweep-live.log`.
...
### 외부/사용자 개입
- [x] ~~(deferred) **Slack App 실 생성/토큰**~~ — **완료(2026-07-19, gate 844)**: 실 App 생성(Webhook `#platform-test`+Interactivity+Signing Secret)→cdk env 주입→알람 트리거→**Approve 버튼 클릭 라이브 E2E**(SFN SUCCEEDED, APR-8BC7E7E95B9A). 부산물: 프로덕션 버그 2건 근본수정(detector NameError·approval_bridge float→Decimal, `0f99420`). 증거 `docs/evidence/slack-interactive-approval-live.log`. 후속: [x] ~~Analyzer `BEDROCK_MODEL_ID` invalid 정정~~ **완료(2026-07-19, `9a56949`)** — 스택 하드코딩 무효 ID→`us.anthropic.claude-sonnet-4-6` 프로파일+정확-ARN IAM, 라이브 `analyzer.llm_done` 검증. · [x] ~~executor `AWS-SendSlackAlert` 유령 SSM 문서~~ **완료(2026-07-19, `55de55e`, 권고안 a)**: `_NOTIFICATION_ACTIONS` in-process 처리, 라이브 P1/AUTO→`resolved=True`(INC-E15BA62E), gate 847. · [ ] (선택) On-Prem 승인 게이트 Slack 버튼 연동.
