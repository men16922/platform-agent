...
- [ ] **⚠️ 결정 5: 배포 요청이 무엇으로 테넌트를 말하는가** — 스코프 가드를 배포 경로에
  태우기 위한 선행 결정. 상세는 아래 잔여 섹션.
- [ ] **(별도 계획) GitAIOps 후속편 아티클** — 논지=책은 AI 자리에 사람이 프롬프트를 넣지만
  ...
  **새 소재**: "선언은 됐는데 아무도 소비하지 않는 코드" 14건(M13) — 전부 테스트는 초록이었고
  라이브만 드러냈다. **수호 테스트 자신이 같은 안티패턴**이던 것(3회)까지 포함.
...
- [ ] **TS 스윕 잔여 후보 — 사문화이지 손실이 아니다(2026-07-29 확인, 고치지 않음)**.
  ...
  substrate/delivery`+`Quota`=**서로만 참조하는 닫힌 섬**. 마지막 건은 "두 번째 진실 소스"
...
- [ ] **인시던트 필드 실 DynamoDB 왕복 미검증(쓰기·읽기 양쪽)** — 모킹 테이블 + 직렬화기 직접
  확인 + 투영 표현식 파싱까지다. 새 속성(`triggered_at`·`confidence`·`reconciliation`·
  `trace_id`·`resolved_at`)이 실 테이블을 왕복해 대시보드에 읽힌 적은 없다 — 실 AWS 승인 사항.
...
- [ ] **⚠️ 배포 경로가 스코프 가드를 안 탄다(2026-07-29 발견, D36에서 분리) — 결정 5**.
  ...
  별개인 **인가** 문제. **선행 항목은 닫혔다**(2026-07-30 `6beebbc`: 행이 착지 네임스페이스를
  기록) — 그런데 **결정은 안 열렸다**: 행은 정직해졌어도 **요청**은 아니다. 프롬프트가
  namespace/tenant를 한 번도 언급하지 않아 모델이 아무것도 안 넘기면 `ServiceSpec`이
  `default`로 해소하고 그건 무테넌트다. 스코프를 켜려면 **먼저 배포 요청이 테넌트를 말하게**
  해야 하고(프롬프트/도구 시그니처/레지스트리 대조 중 무엇으로?), 그게 결정 사항이다.
