...
## 현재 상태 (2026-09-01, gate 2381 — 로컬 macOS·py3.13 · CI도 같은 게이트, ubuntu·py3.13)
...
- [ ] **Phase 4 남은 것 = 4b**(자격증명 파티션 미증명=Risk 10 · 30분 TTL 가드가 상시와 안 맞는다, billable·별 승인) ·
  ...
  - [ ] (선택·소) **Azure 자동 재시도** — 프로브가 `Retry-After`를 지켜 스스로 재시도하려면 **전송 계층을 `az rest`에서 바꿔야** 한다(헤더를 버린다). `test_only_the_cost_query_endpoint_is_posted_to`가 `_run` 인자를 검사하므로 **가드도 같이 옮겨야 한다** ⇒ 별도 항목.
...
- [ ] [auto] Azure Cost Management 429에서 서버가 준 `clienttype-retry-after`를 지켜 자동 재시도. Done: 프로브가 헤더의 대기 시간만큼 기다려 최대 3회 재시도하고, URL 고정 가드(`test_only_the_cost_query_endpoint_is_posted_to`)가 새 전송 계층에 대고도 물어 변이가 red · `make check` 초록.
- [ ] [auto] 스크립트가 인쇄하는 레포-상대 경로의 실존을 전수로 묻는 가드. Done: 테스트 1개가 `scripts/*.py`의 인쇄 경로를 모두 훑고, 없는 경로를 심으면 red(⚠️`json|jsonl` 같은 접두 매칭에 속지 않을 것) · `make check` 초록.
