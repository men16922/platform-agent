...
- [ ] **⚠️`Resource:"*"` 이유 주석 5건 누락(08-15)** — `incident_agent_stack.ts` 7건 중 **적힌 건
  L148·L776 둘뿐**(나머지 L282·L286·**L366**·L635·L871 — L366은 **같은 이유가 L139~141에 이미 있다**).
  **가드 안 만듦**: 5건에 이유를 써야 하는데 AWS 권한 레퍼런스가 JS 렌더라 못 읽었다 —
  **출처 없이 단정하지 않는다.** 재개 = 액션별 리소스 지원 여부 확인.
