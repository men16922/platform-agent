...
## 현재 상태 (2026-08-09, gate 1708 — 로컬 macOS·py3.13, CI 미확인)
...
**남은 건 Phase 4(billable, 별 승인)뿐** — 단 "무과금 작업 소진"은 **08-09에 두 번 틀렸다**
(기록된 이유를 돌려 보니 그때마다 $0 작업이 나왔다). **소진은 목록의 상태지 사실이 아니다.**
...
- [ ] **Phase 4 — 산정 끝, 승인 대기.** 근거는 **`docs/plans/2026-08-08-phase4-scope-and-cost.md`가
  ...
  **≈$5/월** · **4b** 원격 클러스터+DR **≈$185/월**. ⚠️**$0 선행: ₩20 예산은 닫혔고 BQ 결제
  내보내기만 남았다**(콘솔 수동, 사용자 몫) — 없으면 울려도 금액을 모른다. 켠 뒤 먹혔는지는
  **`make spend-check`가 답한다**(GCP 줄이 "아직 못 잰다"에서 바뀐다).
