...
## D51 — **정적검사는 게이트에 들어가지 않는다. 측정만 기록한다**

- **Decision:** `make lint`(ruff)도 `mypy`도 **`make check`에 넣지 않는다**(2026-09-01, 사용자).
  `gate.yml`이 적어 둔 문장을 유지한다 — *"a CI job is a bad place to introduce a standard
  nobody agreed to."* ⚠️**이건 "나중에 보자"가 아니라 측정 위에서 내린 결정이다.**
- **Reason:** 두 후보를 **각각 재고 분류한 뒤** 정했다(M42·M44·M45).
  - **ruff — 잡을 게 없다.** 20건이고, **여섯을 직접 열어** 확인했다: `dep_id`(넘길 파라미터가
    아예 없다) · `azure_runner.url`(각 분기가 자기 URL을 만든다) · F402(문자열 루프 변수와
    이름만 겹침) · `original_guard`(새 인스턴스라 복원할 게 없다) · `result`(단언은 부작용에
    건다) · E701 ×5(열 맞춤). 나머지도 같은 성격 ⇒ **결함 0.** 게이트에 넣으면 **스타일을
    집행하는 것이지 버그를 막는 게 아니다.**
  - **mypy — 신호는 있는데 비율이 나쁘고 비싸다.** 253 중 **200(79%)이 주석 부채**
    (`type-arg`·`import-untyped`·`no-any-return`·`no-untyped-def`·`no-untyped-call`)이고
    **53이 실제 주장**이다. 그 53 중 **21건을 열어 고칠 값이 있던 건 2건**(M44 계약 갭,
    M45 선언 불일치) — 나머지 19는 노이즈이거나 *"코드는 옳고 타입이 못 따라간다"*
    (오탐 4 · 도달 불가 2 · 재-export 3 · 이종 dict 2 · 이름 재선언 1 · 미설치 2).
    그리고 **`mypy src/`는 45초**로 게이트(38초)를 거의 두 배로 만든다 — **D49가 288s→39s를
    얻어낸 이유와 정면으로 충돌한다.**
- **Impact:** 게이트는 그대로 `check: test`다. ⚠️**두 도구가 죽은 건 아니다** — 둘 다 이제
  **돌릴 수 있다**(09-01 전엔 `mypy src/`가 `cdk.out`에서 **수집 단계에 죽었다**, M42).
  선언은 남기되 **집행으로 읽히지 않게** `pyproject.toml`의 주석과
  `tests/test_pyproject_claims.py`가 계속 지킨다.
  ⚠️**이 결정을 "정적검사는 쓸모없다"로 요약 금지** — 이번 세션의 **실제 결함 둘을 찾은 건
  mypy이고 테스트 스위트는 둘 다 놓쳤다**. 결정된 건 *가치*가 아니라 **게이트에 상주시킬
  값어치가 아직 아니라는 것**이고, 근거는 **비율(2/21)과 비용(45초)**이다.
- **여는 조건:** 주석 부채 200이 크게 줄어 53의 비중이 올라가거나, 좁힌 코드군만 도는 방법이
  게이트 시간을 유의미하게 안 늘릴 때. 그때 다시 재는 출발점은
  `docs/evidence/the-signature-said-required-the-body-said-optional.log` **§5**의 분류표다.
...
- **Impact:** DoD ③ 달성 — AMP가 허용목록 4종 **308 시계열**(22·50·220·16)을 그대로 반환하고
  ...
  4a를 접으면 **워크스페이스·IAM 사용자·키 셋 다** 지울 것 — ⚠️**청구액이 $0이라고 이 이유가
  줄지 않는다**(대가는 돈이 아니라 그 키였다). ✅**실제 청구액 측정됨(2026-08-30): $0.00**
  — 계량은 됐고(798,331 샘플) **AMP 프리티어가 `Always Free` 40M/월**이라 값이 0이다.
  ⇒ 위 괄호의 세 금액은 **선택의 근거로는 유효하되 청구액으로는 전부 $0**이고, 비용 절벽은
  요율이 아니라 **40M 한도**로 옮겨 앉았다(필터 없음 5,131M = **128배 초과** → 그때부터 정가).
  권위 = 계획서 **§10** · `docs/evidence/amp-actual-bill-is-zero-and-the-free-tier-reason-was-inverted.log`.
  ✅DoD ①②는 08-17에 닫혔다(M37): 관리형은 매니페스트를 내지 않고 read model이 부재를 설명한다.
- **Folded (2026-09-01, 사용자 결정):** 셋 다 지웠다 — 워크스페이스 `ws-929b8da9…`(8리전
  스윕 후 0) · IAM 사용자 `amp-remote-write-4a` · 키 `AKIA…62VN`. **약속대로 접었다.**
  ⚠️**$0.00이 이유가 아니다** — 위에 미리 적힌 대로 대가는 **그 장기 키 하나**였고, 청구액이
  0이어도 줄지 않았다. ⚠️**D48은 안 죽는다**(*허용목록 없이 remote_write 금지*): 필터 없으면
  5.13B/월 = 프리티어 **128배**, ≈$180/월 = 4b 값이라 **4a를 고른 이유 자체가 지워진다**.
  그래서 `test_amp_cost_handles.py`를 **지우지 않고** 계약을 함수로 빼 합성 표에 물렸다
  (목적지가 없어도 하중 유지, 변이 7종 red). ⚠️**워크스페이스 id는 이제 안 박는다** — 없는
  것을 박으면 영원히 틀릴 수만 있는 규칙이다. ⚠️**KSM interval 60s는 남았고 이유가 바뀌었다**:
  비용 손잡이가 아니라 데모 알람 룰의 `[5m]`=5샘플. 증거
  `docs/evidence/folding-4a-the-price-was-a-long-lived-key.log`.
...
- **Impact:** 4a의 첫 산출물은 인프라가 아니라 **허용목록 결정**이다(계획서 §3이
  *"무엇을 렌더할지는 일부러 발명하지 않았다"*로 남긴 항목 — **이제 비용이 값을 매겼다**).
  ...
  🔴 **정정(2026-08-30, 측정)**: 이 추론의 전건이 거짓이다 — AMP 프리티어는 12개월 한정이
  아니라 **`Always Free`**이고, 켜고 물으니 계정에 그렇게 나타났다. **실제 청구 $0.00.**
  ⚠️그래도 **예산 번역은 폐기하지 않는다** — 한도 40M이 새 절벽이라 "몇 시계열까지"라는
  질문 자체는 그대로 살아 있다. 권위 = 계획서 §10.
