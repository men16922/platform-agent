...
**구현 상태:** Deploy(4-provider) ✅ · Provision(4-provider: On-Prem Terraform/Ansible + AWS CDK + **GCP/Azure GKE·AKS 어댑터, AKS 실 클러스터 라이브**) ✅ · Day-2(AWS/GCP/Azure) ✅ · **Agent Runtime 매니지드 호스팅 어댑터 3/3 클라우드 실 배포 라이브**(AgentCore/Agent Engine/Foundry, create→호출→teardown) ✅ · **MCP Gateway 단일 카탈로그**(게이트웨이+인터랙티브 에이전트, 드리프트-0) ✅ · **Orchestrator(supervisor) 라우팅 + A2A Agent Card discovery/위임 ✅**(실 kagent 라이브) · **agents-as-tools 오케스트레이션 + self-consistency ✅**(Tier 2 #2, `orchestration.py`) · **On-Prem Day-2 실 executor(되돌리기-가능 4조치: restart/undo/scale/polite drain) ✅** · **AWSome AI Gateway 레퍼런스 Tier 1(reconciliation·budget·circuit breaker·cost) + Tier 2(#2 agents-as-tools·#3 MCP-over-HTTP+kill-switch·#4 cross-account STS) 전부 반영 ✅** · **배포 파이프라인↔매니지드 런타임 정면 배선(DeployPipeline `host` 스텝) ✅** · **supervisor를 `local_deploy_api` 정면 진입점으로 배선 ✅**.

> **잔여 로드맵:** ~~① 배포 파이프라인 ↔ 매니지드 런타임 정면 배선~~ **✅ 완료(2026-07-15)** — DeployPipeline에 opt-in `host` 스텝(`get_runtime_adapter(provider).host_agent`, approval-gated preflight/create, onprem=kagent라 skip). ~~② supervisor를 `local_deploy_api` 정면 진입점으로 배선~~ **✅ 완료(2026-07-15)** — `/api/local-deploy`가 supervisor/orchestrator로 먼저 분류→A2A 엔드포인트 설정 시 위임, 미설정 시 in-process deployer 폴백(비파괴). **진짜 미구현(전부 인프라/아스피레이셔널/사용자):** ③ Agent Runtime Memory/Code-Interpreter·Browser 도구(AgentCore 패리티, 현재 stateless) 🔲 · ~~④ On-Prem State Store·Alertmanager 실연동~~ **✅ 완료(2026-07-17)** — 옵트인 `state_store.py`(`PLATFORM_STATE_DSN`)+실 Alertmanager·멀티-레플리카 라이브(`docs/evidence/state-store-alertmanager-live.log`); Redis 캐시는 필요 시 별도 · ~~⑤ Helm/Terraform 프로덕션(레퍼런스 #7)~~ **✅ 전체 완료(2026-07-17)** — Helm(kind 라이브)+Terraform(`aws-production`, validate·apply=사용자) · ⑥ Slack App 실 생성(사용자) · Harbor substrate 🔲. **그 외 코어 아키텍처는 전부 구현·검증(라이브 또는 유닛) 완료.**
...
**경계 판단 — TOOL → SKILL → SUBAGENT smell-test:** 새 능력을 어디에 두는지 세 층으로 판단한다. **TOOL** = 단일 결정론적 동작(부수효과 좁음)이면 MCP 카탈로그 도구. **SKILL(A2A 위임)** = 별도 시스템프롬프트·별도 권한·독립 배포·라이프사이클이 필요하면 A2A 특화 에이전트로 위임(provision/deploy/kagent). **SUBAGENT** = 다단계 자율 루프가 필요할 때만. 기본은 가장 낮은 층. **위임 안전 불변식**(`supervisor.py` docstring): supervisor는 mutating provision/deploy를 **스스로 실행하지 않고 반드시 A2A 경계로 위임**한다(엔드포인트 미설정 시 `delegated=False`, in-process 실행 없음 — 회귀 가드 테스트). 경계를 건너는 호출자 자유텍스트는 **untrusted** → `sanitize_instruction`으로 아웃바운드 bound/clean(control-char strip·length cap, 분류는 원문). 잔여 하드닝 3종(구조화 페이로드·저-confidence 게이트·최소권한 힌트)은 **구현 완료(2026-07-17, 실행 큐 ⑧-1/2/3)** — 설계 이력은 `docs/archive/plans/a2a-delegation-hardening.md`.

**현재 vs 타깃:**

| | 현재 | 타깃 |
|---|---|---|
| 라우팅 | AI Model Router (model × env × role) **+ Orchestrator supervisor (요청 → 에이전트) ✅ + `local_deploy_api` 정면 진입점 배선 ✅**(분류→A2A 위임/in-process 폴백) | (완료) |
...
**On-Prem 구현 (부분 ✅):** 코어 게이트 구현 — `onprem_webhook_api`가 P2(APPROVE) 인시던트를 즉시 실행하지 않고 **파일 기반 pending 스토어**(`onprem_approvals`, offline JSONL)에 parking → `/approve`가 저장된 decision을 executor로 **재생 실행**, `/reject`는 실행 없음, `/pending`은 대기 목록. **대시보드 연동(✅):** Incidents 페이지의 "Pending Remediation Approvals"가 AWS(SFN task token) + On-Prem(webhook `/pending`)을 **hybrid 병합**해 표시(source 배지), Approve/Reject 클릭이 source에 따라 SFN 또는 webhook `/approve`·`/reject`로 라우팅(`ONPREM_WEBHOOK_URL`, 기본 `:8078`; RBAC·감사로그 공통 적용). Slack 버튼 + Temporal/Redis/PostgreSQL substrate는 로드맵.
```
...
| On-Prem | FastAPI endpoint (`local_deploy_api`) | supervisor 프론트도어 → A2A 위임/in-process 폴백 | ✅ 구현 |
...
| 2 | **agents-as-tools 오케스트레이션** — Orchestrator + 전문가(SQL/Validator/Viz/Report) 에이전트, L3 self-consistency | `orchestration.py` · `supervisor.py` · AgentCore(라이브) | **✅ 구현완료** — `orchestration.py`: `route_with_self_consistency()`(N-샘플 majority vote, 저합의 시 결정론적 classifier 폴백=reconciliation 철학) + `Orchestrator`(플랜을 순서대로 `Supervisor.handle`로 위임=specialists-as-tools, 실패 시 short-circuit·shared contextId). a2a_server 옵트인(`SUPERVISOR_ORCHESTRATION`), 기본 무변경 | ✅ Tier 2 |
| 3 | **MCP over HTTP via AgentCore Gateway** — tool_use 가로채기→원격 MCP 호출(SigV4/IRSA)→결과 재주입, 글로벌 kill-switch | MCP Gateway(`mcp_server.py` `TOOL_CATALOG`) | **✅ 구현완료** — `remote_mcp_tool()`: 원격 MCP 서버로 JSON-RPC `tools/call` intercept→reinject(전송 실패 시 error ToolResult로 degrade), `MCPServer(extra_tools=…)`로 카탈로그 등록. **per-tool + 글로벌 kill-switch**(`call_tool` 게이트, `disable_tool`/`set_kill_switch` + `MCP_DISABLED_TOOLS`/`MCP_KILL_SWITCH` env) | ✅ Tier 2 |
| 4 | **Cross-account STS AssumeRole + graceful fallback** — 계정 넘나드는 호출, 실패 시 in-account 폴백 | `adapters/aws_session.py` · 멀티-provider 어댑터 | **✅ 구현완료** — `assume_role_session(role_arn, fallback=...)`: STS `assume_role`로 타깃 계정 세션 구성, 실패/서킷-OPEN 시 in-account 크레덴셜로 **graceful fallback**(Tier 1 `CircuitBreaker` 재사용). `runtime/aws.py` `_client`가 `AWS_ASSUME_ROLE_ARN` 옵트인 소비(미설정=in-account, 무변경) | ✅ Tier 2 |
...
| 7 | **프로덕션 배포** — Helm chart(env×substrate values) + Terraform 모듈(EKS/Aurora/Redis/Cognito/IRSA) | `infra/helm/platform-agent/` + `infra/terraform/aws-production/` | **✅ 구현완료(2026-07-17)** — Helm(차트+이미지, kind 라이브 실증) + Terraform(VPC/EKS/Aurora=State-Store DSN/IRSA, validate 검증·apply=사용자) | ✅ Tier 3 |
...
**✅ Tier 1 반영 완료 (2026-07-15):** #1 Reconciliation gate · #5 예산 3단계 게이트 · #6 회복탄력성(서킷브레이커+readiness) · #8 비용 서브메트릭 — 4종 모두 코드+테스트로 구현·커밋. 핵심은 **#1 Reconciliation gate**: 우리 **Guardian(policy-as-code)** 철학과 정확히 부합하며, 자율 `decide→execute`에서 "LLM이 지어낸 값이 아니라 **실제 진단 도구 결과에만 근거**하도록 강제"해 Day-2 executor가 `ONPREM_EXECUTOR_LIVE`로 실 kubectl을 치는 지금 환각 기반 조치를 차단하는 마지막 방어선이 된다. **✅ Tier 2 반영 완료 (2026-07-15):** #2 agents-as-tools 오케스트레이션(`orchestration.py` self-consistency 라우터 + `Orchestrator`, a2a_server 옵트인, +12 test) · #3 MCP-over-HTTP 커넥터 + per-tool/글로벌 kill-switch(`mcp_server.py` `remote_mcp_tool`/`MCPServer` 게이트, +13 test) · #4 cross-account STS AssumeRole + graceful fallback(`adapters/aws_session.py`, CircuitBreaker 재사용, runtime 어댑터 옵트인, +9 test) — 3종 모두 코드+테스트로 구현·비파괴·옵트인. **✅ #7도 완결(2026-07-17)** — 레퍼런스 8항목 전부 소화.
