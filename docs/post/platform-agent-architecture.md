...
- **Injectable seams, offline-testable.** Transports, STS clients, samplers, clocks, and card-fetchers are all injectable. There's no moto, no live-cloud requirement in the unit suite — a fake is monkeypatched into a module-level seam. That's why 736 tests run in minutes.
...
- **`make check` → 736 passed, 1 skipped.** Every feature lands with tests, and the gate runs on every multi-file change.
